...
use Jartaud\LaravelBraintreeSubscription\Invoice;
...
trait Billable
{
    ...
    public function charge($amount, array $options = []): Successful
    {
        ...
        if (! $response->success) {
            throw new Exception(__('Braintree was unable to perform a charge') . ': ' . $response->message);
        }
     ...
     * @param  string  $plan
     * @return \Jartaud\LaravelBraintreeSubscription\SubscriptionBuilder
     */
     ...
     *
     * @param  string  $subscription
     * @return \Jartaud\LaravelBraintreeSubscription\Subscription|null
     */
     ...
     *
     * @param  string  $id
     * @return \Jartaud\LaravelBraintreeSubscription\Invoice|null
     */
     ...
     *
     * @param  string  $id
     * @return \Jartaud\LaravelBraintreeSubscription\Invoice
     */
    ...
    public function updateCard($token, array $options = [])
    {
        ...
        if (! $response->success) {
            throw new Exception(__('Braintree was unable to create a payment method') . ': ' . $response->message);
        }
    ...
    public function applyCoupon($coupon, $subscription = 'default', $removeOthers = false)
    {
        ...
        if (! $subscription) {
            throw new InvalidArgumentException(__('Unable to apply coupon. Subscription does not exist.'));
        }
    ...
    public function createAsBraintreeCustomer($token, array $options = []): Customer
    {
        ...
        if (! $response->success) {
            throw new Exception(__('Unable to create Braintree customer') . ': ' . $response->message);
        }
